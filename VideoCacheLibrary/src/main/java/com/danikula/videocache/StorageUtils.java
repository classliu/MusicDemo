...
final class StorageUtils {
    ...
    private static final String WAWAYAYAREADER_DIR_NAME = "wawayayareader";
    private static final String INDIVIDUAL_DIR_NAME = "mp3-cache";

    /**
     * 2017-5-4
     *  存储修改为　wawayayareader/mp3-cache 目录下
     *  file:///storage/emulated/0/wawayayayreader/mp3-cache/
     * @return
     */
    public static File  getIndividualCacheDirectory(){
        File dataDir = new File(Environment.getExternalStorageDirectory(),WAWAYAYAREADER_DIR_NAME);
        File mp3CacheDir = new File(dataDir,INDIVIDUAL_DIR_NAME);
        if(!mp3CacheDir.exists()){
            if (!mp3CacheDir.mkdirs()) {
