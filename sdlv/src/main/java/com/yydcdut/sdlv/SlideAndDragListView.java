...
public class SlideAndDragListView<T> extends DragListView<T> implements WrapperAdapter.OnAdapterSlideListenerProxy,
    ...
    /* 可以侧滑 */
    private boolean mCanSilde = true;
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View v, int position, long id) {
        ...
        if (mState == STATE_LONG_CLICK_FINISH || mState == STATE_DOWN) {
            if (mCanDrag) startDrag(position);
        }
        return false;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        switch (ev.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (fingerLeftAndRightMove(ev) && !fingerTouchMoveView(ev)) {//上下范围在50，主要检测左右滑动
                    Log.e("-fingerTouchMoveView--", "" + !fingerTouchMoveView(ev));
                    return true;
                }
                break;
        }
    ...
    /**
     *  在headView 和 footView上可以由控件决定滑动
     * @param  ev
     * @return
     */
    private boolean fingerTouchMoveView(MotionEvent ev) {
        int position = pointToPosition((int) ev.getX(), (int) ev.getY());
        if (position != AdapterView.INVALID_POSITION) {
            boolean isHeaderView = false;
            int headerViewCount = getHeaderViewsCount();
            if (headerViewCount >= 1) {
                isHeaderView = position <= (headerViewCount - 1);
            }
            boolean isFootView = false;
            int footView = getFooterViewsCount();
            if (footView >= 1) {
                isFootView = (position >= (getChildCount() - 1 - footView));
            }
            return isHeaderView || isFootView;
        }
        return false;
    }
    ...
    public void setNormalListView() {
        setCanSlideAndDrag(false, false);
    }
     ...
     * @param canSilde
     */
    public void setCanSlideAndDrag(boolean canDrag, boolean canSilde) {
        mCanDrag = canDrag;
        mCanSilde = canSilde;
    }
